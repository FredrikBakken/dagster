...
from google.cloud.dataproc_v1.types.jobs import Job, SubmitJobRequest
...
@public
@experimental
class PipesDataprocClient(PipesClient, TreatAsResourceParam):
    """A pipes client for running jobs on GCP Dataproc.

    Args:
        client (Optional[google.cloud.dataproc.Client]): The GCP Dataproc client used to interact with GCP Dataproc.
    ...
    def _enrich_submit_job_params(
        ...
        job.labels = {**(job.labels or {}), **session.default_remote_invocation_info}
    ...
    def _get_project_id_and_region(
        ...
        return (
            params.get("project_id") or params.get("request", {}).get("project_id"),
            params.get("region") or params.get("request", {}).get("region"),
        )
